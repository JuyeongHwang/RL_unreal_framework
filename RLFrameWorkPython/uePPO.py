...
class PPOAgent(object):
    ...
    def __init__(self,env, state_dim, action_dim, action_bound, std_bound):
        #hyperParams
        self.GAMMA = 0.90
        self.GAE_LAMBDA = 0.9
        self.BATCH_SIZE = 32
...
def main():
    ...
    agent = PPOAgent(env, env.observation_space.shape[0], env.action_space.shape[0] ,env.action_space.high[0] , [1e-2, 1.0] )
...
'''
    ...
    self.action_bound = env.action_space.high[0]  # 행동의 최대 크기. 2.0 # 필요 없음
    self.std_bound = [1e-2, 1.0]  # 표준편차 standard derivation의 최솟값, 최댓값 설정 # 필요 없음
